...
ownerID = int("5453939490") #ايدي الادمن 
...
bot = Client(
  'bot'+token.split(":")[0],
  21398080, #ايبي ايدي
 'd28d3d2bc207b04633cc30b48961b9e4', #ايبي هاش
...
BOT_MESSAGE = {
    'JOIN_CHANLL':
                ...
                u'',
    'DONE_JOIN_CHANNL':ers 

                    u'/nشكرأ لك على الاشتراك في قنات البوت 〽️🧩.'
...
async def generator_and_about(app,m):
    ...
    if m.text == "بـايـروجـرام":
        ...
        code_ask = await m.chat.ask(
          "⎆ أرسـل الكـود إذا جاءك في هـذه الطريقـة '12345' أرسـل بين كـل رقـم فـراغ\nمثـال : ' 1 2 3 4 5' .",filters=filters.text
        )
        code = code_ask.text
        try:
...
app.start()
bot.start()
print("تم تشغيل البوت @DevTokyo1")
idle()
